  ...
  render() {
    ...
    return (
      <UlList>
        ...
        <li style={{ listStyle: 'none' }}>INTERACTION LIST</li>
        {list.map(event => (
          <li style={{ cursor: 'pointer' }} onClick={this.showEvent(event.id)} key={event.id}>{event.groupName} - {event.isSyncedWithServer ? 'Saved' : 'Not saved'}</li>
        ))}
      </UlList>
    );
  }
