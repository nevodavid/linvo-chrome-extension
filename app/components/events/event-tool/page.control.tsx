...
class PageControl extends Component<PageControlProps> {
    ...
    render() {
        const {events, eventGroup, widgets, user} = this.props;
        return !eventGroup || !user ? <div/> : (
            <div>
                <EventToolLayout/>
                ...
                <LeftBarComponent>
                    <div style={{display: 'flex', flexDirection: 'row'}}>
                        <div style={{display: 'flex', flexDirection: 'row', flex: 1}}>
                            {events.map(e => (
                                <Event deleteEvent={this.deleteEvent(e.id)} {...e}>{e.url}</Event>
                            ))}
                            <AddEventDiv onClick={this.add}>+ ADD EVENT</AddEventDiv>
                        </div>
                        <Input onChange={this.changeGroup} style={{fontSize: 30}} value={eventGroup.groupName} type="text"/>
